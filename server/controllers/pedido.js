...
// CHAVE API KEY - PEGUEI ESSA API NO SITE "Geoapify"
const key = 'bd01abc8f4754747bfbf69746fbc0593';
...
const controllers = () => {
    ...
    // obtem a rota (latitude e longitude) e calcula a taxa do delivery por KM 
    const calcularTaxaDelivery = async (req) => {

        try {


            // primeiro: obtem (latitude e longitude) do endereço da empresa
                ...
                // segundo: obtem (latitude e longitude) do endereço do cliente
                ...
                const distancia = await calcularDistancia(
                    ...
                    // obtem qual taxa e mais adequada para essa distancia
    ...
    const calcularDistancia = async (lat, lon, latLoja, lonLoja) => {
        ...
        try {
            ...
            return {
                status: 'error',
                message: 'Falha ao obter a localização. por favor, selecione outro endereço ou altere o atual.',
                ex: error
            }
    ...
    const obterPedidoPorId = async (req) => {
        ...
        try {
         ...
         // se for pelo painel, retornamos o carrinho tbem
            if (painel) {
    ...
    const obterPedidoPorStatus = async (req) => {
        ...
        try {
            ...
            // se for os concluidos, retorna somente os 20 ultimos pedidos 
            if (idpedidostatus == 5) {

                
            var ComandoSql = await readCommandSql.retornaStringSql('obterPedidosConcluidos', 'pedido');
            ...
            // Alem disso, já obtem os totais de cada TABS e manda no retorno
    ...
    const recusarPedido = async (req) => {
        ...
        try {
                ...
                var ComandoSql = await readCommandSql.retornaStringSql('recusarPedido', 'pedido');
                await db.Query(ComandoSql, { idpedido: req.body.idpedido, motivo: req.body.motivo });
