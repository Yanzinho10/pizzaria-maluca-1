...
const controllers = () => {
    ...
    const duplicarCategoria = async (req) => {
        ...
        try {
            ...
            // altera o nome para "copia" e insere a nova categoria no "banco"
            ...
            if (nova_categoria.insertId != undefined && nova_categoria.insertId > 0) {
                ...
                // se deu certo o cadastro e retornou o ID da categoria cadastrada, pecorre os produtos e adiciona na nova categoria
                const promises = await produtos_categoria.map(async elem => {

                    // cria um ID baseado na data e hora (para ser unico), para passar pra imagem na ora de copiar
                    ...
                    //adicionar o produto na nova categoria
                       var ComandoSqlAddProduto = await readCommandSql.retornaStringSql(metodoImagem, 'produto');
                    ...
                    if (imagemOld != null) {
                        //  faz uma copia da imagem para a pasta
    ...
    const removerCategoria = async (req) => {
        ...
        try {
            ...
            var ComandoSqlRemoveProdutos = await readCommandSql.retornaStringSql('removerPorCategoriaId', 'produto');
            await db.Query(ComandoSqlRemoveProdutos, {idcategoria: idcategoria });


            // depois, remove a categoria
            var ComandoSqlRemoveCategoria = await readCommandSql.retornaStringSql('removerPorId', 'categoria');
