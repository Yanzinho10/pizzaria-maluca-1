...
const controllers = () => {
    ...
    const copy = async (imagem, id) => {
        ...
        try {
            ...
            // cria um "modelo" de imagem, com ID na frente (passado nos parametros)
            var outStr = fs.createWriteStream(`server/public/images/${id}-${imagem}`);


            // aqui faz a copia da imagem para o modelo, com o ID na frente para diferenciar
            inStr.pipe(outStr);
    ...
    const uploadLogo = async (req) => {
        ...
        try {
            ...
            mv(imagem.path, new_path, {
                mkdirp: true //se não existir cria o diretorio
            }, (err, result) => {
                if (err) {
      ...
      const removeLogo = async (req) => {
        ...
        try {
            ...
            return {
                status: 'error',
                message: 'Falha ao remover a imagem. Tente novamente'
            }
        }
    ...
    const uploadImagemProduto = async (req) => {
      ...
      const removeImagemProduto = async (req) => {
        ...
        try {
            ...
            var ComandoSqlObterPorId = await readCommandSql.retornaStringSql('obterPorId', 'produto');
