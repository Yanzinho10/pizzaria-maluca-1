...
cardapio.method = {
        ...
        if (list.length > 0) {
            ...
            list.forEach((e, i) => {
               ...
               if (_icone.length > 0) {
                ...
                // adiciona a categoria no cardapio
                document.querySelector('#listaItensCardapio').innerHTML += tempHeaderCategoria;

                // no ultimo item, obtem os produtos
                if (list.length == (i + 1)) {

                    cardapio.method.obterProdutos();

                    // inicia a validação do scroll para setar a ccategoria ativa
    ...
    // metodo para abrir os detalhes do produto
    abrirProduto: (id) => {
    ...
    validarCategoriaScroll: () => {
        ...
        for (let index = 0; index < categorias.length; index++) {
            ...
            let docViewTop = window.scrollY;  // valor do scroll da pagina atualmente
            let elemTop = document.querySelector('#' + element).offsetTop;  // posição do header da categoria em relação ao topo
            let top = (elemTop - (docViewTop + 100)) * -1;  // faz a conta para validar se estár no topo. obs: o 100 e o valor do menu superior
            let id = element.split('categoria-header-')[1];  // pega o ID da categoria

            // se for > 0, quer dizer que ta no topo. ativa a categoria
            if (top > 0) {
    ...
    selecionarCategoria: (id) => {
        ...
        // metodo para scrolar a pagina até o elemento
        window.scrollTo({
...
cardapio.templates = {
    ...
    categoria:  `
     ...
     <a href="#!" id="categoria-\${idcategoria}" class="item-categoria btn btn-white btn-sm mb-3 me-3 animated fadeIn \${active}" onclick="cardapio.method.selecionarCategoria('\${idcategoria}')"  >
            \${icone}&nbsp; \${nome}
     </a>
