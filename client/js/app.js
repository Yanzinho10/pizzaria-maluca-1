...
app.method = {
            ...
            if(app.method.validaToken(login)) {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (this.readyState == 4) {
                        ...
                        // se o retorno for não autorizado, redireciona o usuario para o login 
                            if (xhr.status == 401) {
                                app.method.logout();
                            }
    ...
    // metodo para validar se o token existe (local). E chamado em todas as requisições
   ...
   // metodo que limpa todo o localstorage e redireciona pro login 
    logout: () => {
        localStorage.clear();
        window.location.href = '/painel/login.html';
        ...
        app.method.get('/empresa/open', 
            (response) => {
                ...
                if (response.status == "error") {
                    ...
                    // alterar o label aberto/fechado (se tiver na tela principal do cardapio)
                    if (home) {
