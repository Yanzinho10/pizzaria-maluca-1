            ...
            list.forEach((e, i) => {
                ...
                if (e.opcionais.length > 0) {
                    ...
                    for (let index = 0; index < e.opcionais.length; index++) {
                        ...
                        itens += carrinho.template.opcional.replace(/\${nome}/g, `${e.quantidade}x ${element.nomeopcional}`)//aqui altera os titulos do pedido se eu quiser, e so remover o "element.titulo dessa linha"
...
// ------- FIM DO ITENS DO CARRINHO -------



// -------- INICIO DO TIPO DE ENTREGA - DELIVERY OU RETIRADA --------
...
obtertiposEntrega: () => {
    ...
    app.method.get('/entrega/tipo',
        (response) => {
            ...
            // valida se o delivery (entrega) esta ativo 
            if (delivery[0].ativo) {
            ...
            // valida se a retirada esta ativa
            if (retirada[0].ativo) {
...
// obtem as taxas de entregas deffinidas
obterTaxaEntrega: () => {
...
validarEnderecoSelecionado: () => {
    ...
    if (TAXAS_ENTREGAS.length == 0) {
        document.querySelector("#containerTaxaEntrega").classList.add('hidden');
        return;
    }

    // Taxa Única
    if (TAXAS_ENTREGAS[0].idtaxaentregatipo == 1) {

        TAXA_ATUAL = TAXAS_ENTREGAS[0].valor;
        TAXA_ATUAL_ID = TAXAS_ENTREGAS[0].idtaxaentrega;


        let tempo = '';

        if ((TAXAS_ENTREGAS[0].tempominimo != null && TAXAS_ENTREGAS[0].tempominimo > 0) &&
          (TAXAS_ENTREGAS[0].tempomaximo != null && TAXAS_ENTREGAS[0].tempomaximo > 0)) {
            tempo = `(${TAXAS_ENTREGAS[0].tempominimo}-${TAXAS_ENTREGAS[0].tempomaximo}min)`;
        }
    ...
    // Taxa porDistancia
    if (TAXAS_ENTREGAS[0].idtaxaentregatipo == 2) {

        // valida se e a Retirada se ta checada
        ...
        if (enderecoAtual != undefined) {
            ...
            // obtem a diferença da distancia entre a loja e o endereço selecionado
            let dados = {
            ...
            //  seta o tempo minimo e maximo da entrega
             let filtro_taxa = TAXAS_ENTREGAS.filter((e) => { return e.idtaxaentrega == TAXA_ATUAL_ID });
    ...
    // Sem Taxa
    if (TAXAS_ENTREGAS[0].idtaxaentregatipo == 3) {
        TAXA_ATUAL = 0;

        // seta o tempo minimo e maximo default
...
// -------- FIM DO TIPO DE ENTREGA - DELIVERY OU RETIRADA --------
...
// abre a modal para informa o endereço
abrirModalEndereco: () => {
...
salvarEndereco: () => {
 ...
 buscarCep: () => {
    ...
    if (cep != '') {
        ...
        // expressão regularpara validar o CEP
        var validacep = /^[0-9]{8}$/;
...
// abre a modal para 'editar' ou 'remover o endereço'
...
selecionarFormaPagamento: (forma) => {
    ...
    if (selecionada.length > 0) {
        ...
        // se for DINHEIRO
            ...
            let troco = prompt("Informe o valor do troco?");
            if (troco != null) {

                // valida se o troco esta correto
                ...
                if (isNaN(_teste) || troco.trim() == '' || _teste <= 1 ) {
                    ...
                    TROCO = _teste
                    document.querySelector('#lblDescFormaPagamentoSelecionada').innerText = `Troco para: ${(_teste).toFixed(2).replace('.', ',')} reais.`;
                }
...
// -------- FIM DO FORMAS DE PAGAMENTO ----------




// -------- INICIO DO REALIZAR PEDIDO----------
...
fazerPedido: () => {
    ...
    if (CARRINHO_ATUAL.length > 0) {
        ...
        if (celular.length <= 0) {
            app.method.mensagem("Informe o Número do Celular, por favor.");
            return;
        }
        ...
        var dados = {
            ...
            idtaxaentregatipo: TAXAS_ENTREGAS[0].idtaxaentregatipo,
