...
pedido.method = {
        ...
        if (lista.length > 0) {
            ...
            lista.forEach((e, i) => {
                ...
                if (e.idpedidostatus == 1) {
                    titleBtn = 'Pendente'; //se de problema "bota o 'P' pra minusculo"
                    acoes = `

                    <a href="#!" onclick="pedido.method.moverPara(2, '${e.idpedido}')" class="dropdown-item">Mover para <b>Aceito</b> <i class="far fa-thumbs-up"></i></a>
                    <a href="#!" onclick="pedido.method.moverPara(3, '${e.idpedido}')" class="dropdown-item">Mover para <b>Em Preparo</b> <i class="far fa-clock"></i></a>
                    <a href="#!" onclick="pedido.method.moverPara(4, '${e.idpedido}')" class="dropdown-item">Mover para <b>Em Entrega</b> <i class="fas fa-motorcycle"></i></a>
                    <a href="#!" onclick="pedido.method.moverPara(5, '${e.idpedido}')" class="dropdown-item">Mover para <b>Concluido</b> <i class="far fa-check-circle"></i></a>
                    <a href="#!" onclick="pedido.method.moverPara(6, '${e.idpedido}')" class="dropdown-item">Recursar Pedido <i class="far fa-times-circle"></i></a> `
                ...
                if (e.idpedidostatus != 5 && e.idpedidostatus != 6) {
                    ...
                    btnAcoes = `
                     ...
                     <button class="btn btn-white btn-sm dropdown-toggle active" 
                         type="button" data-bs-toggle="dropdown" aria-expanded="false" id="menuAcoes">
                         ${titleBtn}                           
                    </button> `;
    ...
    carregarModalDetalhes: (data, idpedido, cart) => {
        ...
        // organiza os carrinhos em grupo
        ...
        for (var key in itens_pedido) {
            ...
            // adiciona o item no objeto de ordem
            order.push(_item) 
        ...
        order.forEach((e, i) => {
            ...
            if (e.opcionais.length > 0) {
                ...
                for (let index = 0; index < e.opcionais.length; index++) {
                    ...
                    itens += pedido.template.opcional.replace(/\${nome}/g, `${e.quantidade}x ${element.nomeopcional}`)//aqui altera os titulos do pedido se eu quiser, e so remover o "element.titulo dessa linha"
    ...
    // metodo chamado para atualizar a lista de pedidos de acordo com a TAB selecionada
    atualizarLista: () => {

        // valida qual e o target, pra carregar os itens da tab atual
