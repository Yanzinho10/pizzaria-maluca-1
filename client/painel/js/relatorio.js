...
relatorio.method = {
    ...
    bloquearDatasFiltrosFaturamento: () => {
        ...
        // a data inicial podera ser selecionada até 1 ano atras
        ...
        // a data da seleção final pode ser somente até o ultimo dia do mês atual
    ...
    carregarDataAtualFiltroFaturamento: () => {
        ...
        let diaIni = firstDate.getDate();
        let mesIni = firstDate.getMonth() + 1;
        let anoIni = firstDate.getFullYear();
    ...
    // filtra os faturamentos de acordo com os filtros
    filtrarFaturamento: () => {
        ...
        if (datainicio == '') {
            app.method.mensagem('Informe uma data de Inicio válida.')
            return;
        }

        if (datafim == '') {
            app.method.mensagem('Informe uma data de Fim válida.')
            return;
        }
    ...
    carregarGrafico: (lista) => {
        ...
        if (lista.length > 0) {
            ...
            // percorre a lista para montar o objeto do grafico
            var LINHAS = [];
            var VALORES = [];
            ...
            for (let index = 0; index < diff_days; index++) {
                ...
                // valida se tem registro pra essa data na lista que retornou da API
    ...
    // -------- HISTORICO -----------
     ...
     bloquearDatasFiltrosHistorico: () => {
        ...
        // a data da seleção final pode ser somente até HOJE
    ...
    listarPedidos: (list) => {
        ...
        if (list.length > 0) {
             ...
             $.each(list, (i, e) => {
                ...
                // quando for o ultimo item, renderiza a tabela na tela (DataTable)
                if ((i + 1) == list.length) {
