...
cardapio.event = {
    ...
    init: () => {
        ...
        // mascara para o campo de dinheiro
        ...
        // inicializa o drag e o drop da imagem
        ...
        // evento quando passa o mousa encima com a imagem segurada (hover)
        ['dragenter', 'dragover'].forEach(eventName => {
            DROP_AREA.addEventListener(eventName, cardapio.method.highlight, false);
        });


         // evento quando sai com o  mousa de encima 
...
cardapio.method = {
    ...
    carregarCategorias: (lista) => {
        ...
        if (lista.length > 0) {
            ...
            lista.forEach((e, i) => {
                ...
                // ultimo item, inicia o evento de tooltip
                if ((i + 1) == lista.length) {
    ...
    // obtem os produtos, quando clica em expandir a categoria 
    ...
    // metodo que atualiza as ordens da categoria 
    atualizarOrdemCategoria: () => {
        ...
        $.each(listaCategorias, (i, e) => {
            ...
            // ultimo item, manda as informaçoes para a API 
    ...
    // metodo para abrir modal de adicionar nova categoria
    ...
    // metodo para duplicar a categoria
    duplicarCategoria: () => {
     ...
     // metodo para remover a categoria
     removerCategoria: () => {
    ...
    editarCategoria: (idcategoria) => {
        ...
        if (categoria.length > 0) {
            ...
            // altera os campos os campos da modal
    ...
    // metodo para confirmar o cadastro / edição da categoria
    salvarCategoria: () => {
    ...
    // metodo para abrir modal de adicionar novo produto
    ...
    // metodo para abrir a modal de editar produto
    editarProduto: (idcategoria, idproduto) => {
        ...
        if (produto.length > 0) {
             ...
             // limpa os campos
                ...
                $("#txtPreçoProduto").val((produto[0].valor).toFixed(2).toString().replace('.', ','));
    ...
    // metodo para confirmar o cadastro / edição do produto
    salvarProduto: () => {

        // validar os campos
        let nome = $("#txtNomeProduto").val().trim();
        let valor = parseFloat($("#txtPreçoProduto").val().replace(/\./g, '' ).replace(',', '.'));
    ...
    // metodo para duplicar o produto
    duplicarProduto: () => {
    ...
    // abre a modal de adicional imagens do produto
    ...
    // DRAG AND DROP - remove a classe 'highlight' quando entra com a imagem no container
    unhighlight: (e) => {
...
carregarOpcionaisProduto: (lista) => {
    ...
    if (lista.length > 0) {
        ...
        // agrupa pelo tipo seleção (opcionais de seleção)
        ...
        Object.entries(listaAgrupada).forEach((e, i) => {
            ...
            // valida se e obrigatorio ou não e altera o subtitulo
...
// abre ou fecha a modal para cadastra um novo opcional
abrirModalAddOpcional: () => {
    ...
    $("#text-opcao-hint").text('Escolha até uma opção');
...
salvarOpcional: () => {
    ...
    if (opcoes) {
        ...
        if (tituloSecao.length <= 0) {
            app.method.mensagem('Informe o titulo da seção, por favor.');
            return;
        }


        if (minimoOpcao.length <= 0) {
            app.method.mensagem('Informe o minimo, por favor.');
            return;
        }

        if (maximoOpcao.length <= 0) {
            app.method.mensagem('Informe o maximo, por favor.');
            return;
        }
        ...
        if (!continuar) {
            app.method.mensagem('Alguns campas não foram preenchidos.');
             return;
        }
...
// metodo que envia a requisição para API (salvra o opcional)
...
// metodo chamado quando digita algo no campo de titulo da seção
...
// metodo chamado quando aumenta ou diminui a opção de MINIMO E MAXIMO
...
// atualiza o texto que mostra como vai ficar no cardapio
atualizarHintOpcao: () => {
...
// fecha a modal de ADD opcionais e re-abre a modal de lista dos opcionais
